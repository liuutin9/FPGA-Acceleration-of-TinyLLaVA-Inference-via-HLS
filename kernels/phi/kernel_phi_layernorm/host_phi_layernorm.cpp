...
int main(int argc, char* argv[])
{
	...
	if (posix_memalign(&ptr, 4096, SIZE_BIAS * sizeof(uint16))) {
	...
	OCL_CHECK(errCode, buffer_phi_layernorm_out = clCreateBuffer(Context, CL_MEM_WRITE_ONLY | CL_MEM_USE_HOST_PTR, SIZE_OUT * sizeof(fixed32_14), OUT, &errCode));
	OCL_CHECK(errCode, buffer_phi_layernorm_in = clCreateBuffer(Context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, SIZE_IN * sizeof(fixed32_14), IN, &errCode));
	OCL_CHECK(errCode, buffer_phi_layernorm_weight = clCreateBuffer(Context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, SIZE_WEIGHT * sizeof(uint16), WEIGHT, &errCode));
	OCL_CHECK(errCode, buffer_phi_layernorm_bias = clCreateBuffer(Context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, SIZE_BIAS * sizeof(uint16), BIAS, &errCode));
