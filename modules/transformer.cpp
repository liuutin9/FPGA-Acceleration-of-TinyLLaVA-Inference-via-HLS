...
void transformer_block(int out[M][N], int in[M][N]) {
    ...
    int wq[N][N] = {0}, wk[N][N] = {0}, wv[N][N] = {0};
    int Q[M][N], K[M][N], V[M][N], attention_out[M][N], add_out[M][N], ffn_in[M][N], ffn_out[M][N];
    float layer_norm_in[M][N], layer_norm_out[M][N];
    qkv_generator<M, N, N>(Q, K, V, in, wq, wk, wv);
    attention<M, N>(attention_out, Q, K, V);
    matadd<M, N>(add_out, attention_out, in);
    for (int i = 0; i < M; i++) {
        for (int j = 0; j < N; j++) {
    ...
    feed_forward<M, N>(ffn_out, ffn_in);
    matadd<M, N>(add_out, ffn_out, ffn_in);
    for (int i = 0; i < M; i++) {
        for (int j = 0; j < N; j++) {
            layer_norm_in[i][j] = 0.0 + add_out[i][j];
        }
    ...
    for (int i = 0; i < M; i++) {
        for (int j = 0; j < N; j++) {
            out[i][j] = (int) std::floor(layer_norm_out[i][j]);
        }
    }
}