...
void run_custom_profiling (int Nb_Of_Kernels, int Nb_Of_Memory_Tranfers, cl_event* K_exe_event, cl_event* Mem_op_event,string* list_of_kernel_names) {
	...
	PROFILE[0].action_type="kernel";         PROFILE[0].name="K_Siglip_layernorm";          PROFILE[0].event = K_exe_event[0];
...
#define SIZE_SIGLIP_ENCODER_LAYERNORM_IN       INPUT_DIM * HIDDEN_SIZE
#define SIZE_SIGLIP_ENCODER_LAYERNORM_WEIGHT   HIDDEN_SIZE
#define SIZE_SIGLIP_ENCODER_LAYERNORM_BIAS     HIDDEN_SIZE
#define SIZE_SIGLIP_ENCODER_LAYERNORM_OUT      INPUT_DIM * HIDDEN_SIZE
...
int main(int argc, char* argv[])
{
	...
	// -------------------------------------------------------------
	cl_kernel K_Siglip_layernorm;

	#ifdef ALL_MESSAGES
	cout << "HOST-Info: Creating a Kernel: K_Siglip_layernorm ..." << endl;
	#endif
	K_Siglip_layernorm = clCreateKernel(Program, "Siglip_layernorm", &errCode);
	if (errCode != CL_SUCCESS) {
		cout << endl << "HOST-Error: Failed to create K_Siglip_layernorm" << endl << endl;
		return EXIT_FAILURE;
	}
	...
	if (posix_memalign(&ptr,4096,SIZE_SIGLIP_ENCODER_LAYERNORM_IN*sizeof(fixed16_10))) {
		cout << endl << "HOST-Error: Out of Memory during memory allocation for in array" << endl << endl;
		return EXIT_FAILURE;
	}
	in = reinterpret_cast<fixed16_10*>(ptr);
	const char* filename = "/user/undergraduate/zylin24/Test_data/Siglip_embedding_output.bin";
	load_weight(filename, in, SIZE_SIGLIP_ENCODER_LAYERNORM_IN);
	cout << "Generated " << SIZE_SIGLIP_ENCODER_LAYERNORM_IN << " values" << endl;

	cout << "HOST-Info: Generating data for weight ... ";
	if (posix_memalign(&ptr,4096,SIZE_SIGLIP_ENCODER_LAYERNORM_WEIGHT*sizeof(fixed16_10))) {
	...
	filename = "/user/undergraduate/zylin24/Test_data/weight/vision_tower._vision_tower.vision_model.encoder.layers.0.layer_norm1.weight.bin";
	load_weight(filename, weight, SIZE_SIGLIP_ENCODER_LAYERNORM_WEIGHT);
	cout << "Generated " << SIZE_SIGLIP_ENCODER_LAYERNORM_WEIGHT << " values" << endl;

	cout << "HOST-Info: Generating data for bias ... ";
	if (posix_memalign(&ptr,4096,SIZE_SIGLIP_ENCODER_LAYERNORM_BIAS*sizeof(fixed16_10))) {
		cout << endl << "HOST-Error: Out of Memory during memory allocation for bias array" << endl << endl;
		return EXIT_FAILURE;
	}
	bias = reinterpret_cast<fixed16_10*>(ptr);
	filename = "/user/undergraduate/zylin24/Test_data/weight/vision_tower._vision_tower.vision_model.encoder.layers.0.layer_norm1.bias.bin";
	load_weight(filename, bias, SIZE_SIGLIP_ENCODER_LAYERNORM_BIAS);
	cout << "Generated " << SIZE_SIGLIP_ENCODER_LAYERNORM_BIAS << " values" << endl;

	cout << "HOST-Info: Allocating memory for RES    ... ";
	if (posix_memalign(&ptr,4096,SIZE_SIGLIP_ENCODER_LAYERNORM_OUT*sizeof(fixed16_10))) {
	...
	// .......................................................
	GlobMem_BUF_in = clCreateBuffer(Context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, SIZE_SIGLIP_ENCODER_LAYERNORM_IN * sizeof(fixed16_10), in, &errCode);
	if (errCode != CL_SUCCESS) {
		cout << endl << "Host-Error: Failed to allocate Global Memory for GlobMem_BUF_DataIn" << endl << endl;
		return EXIT_FAILURE;
	}
	GlobMem_BUF_weight = clCreateBuffer(Context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, SIZE_SIGLIP_ENCODER_LAYERNORM_WEIGHT * sizeof(fixed16_10), weight, &errCode);
	if (errCode != CL_SUCCESS) {
		cout << endl << "Host-Error: Failed to allocate Global Memory for GlobMem_BUF_DataIn" << endl << endl;
		return EXIT_FAILURE;
	}
	GlobMem_BUF_bias = clCreateBuffer(Context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, SIZE_SIGLIP_ENCODER_LAYERNORM_BIAS * sizeof(fixed16_10), bias, &errCode);
	if (errCode != CL_SUCCESS) {
		cout << endl << "Host-Error: Failed to allocate Global Memory for GlobMem_BUF_DataIn" << endl << endl;
		return EXIT_FAILURE;
	}
	GlobMem_BUF_out = clCreateBuffer(Context, CL_MEM_WRITE_ONLY | CL_MEM_USE_HOST_PTR, SIZE_SIGLIP_ENCODER_LAYERNORM_OUT * sizeof(fixed16_10), out, &errCode);
	if (errCode != CL_SUCCESS) {
	...
	errCode |= clSetKernelArg(K_Siglip_layernorm,         0, sizeof(cl_mem),  &GlobMem_BUF_out);
	errCode |= clSetKernelArg(K_Siglip_layernorm,         1, sizeof(cl_mem),  &GlobMem_BUF_in);
	errCode |= clSetKernelArg(K_Siglip_layernorm,         2, sizeof(cl_mem),  &GlobMem_BUF_weight);
	errCode |= clSetKernelArg(K_Siglip_layernorm,         3, sizeof(cl_mem),  &GlobMem_BUF_bias);
	...
	if (errCode != CL_SUCCESS) {
		cout << endl << "Host-Error: Failed to write DataIn_weight to GlobMem_BUF_DataIn_weight" << endl << endl;
		return EXIT_FAILURE;
	}
	...
	cout << "HOST-Info: Submitting Kernel K_Siglip_layernorm ..." << endl;
	errCode = clEnqueueTask(Command_Queue, K_Siglip_layernorm, 0, NULL, &K_exe_event[0]);
	if (errCode != CL_SUCCESS) {
		cout << endl << "HOST-Error: Failed to submit K_Siglip_layernorm" << endl << endl;
		return EXIT_FAILURE;
	}
	...
	// ------------------------------------------------------
	char Output_File_Name[] = "/user/undergraduate/zylin24/Test_data/Siglip_encoder_layer0_layernorm_output.txt";
	...
	for (int i=0; i < SIZE_SIGLIP_ENCODER_LAYERNORM_OUT; i++) {
	...
	string list_of_kernel_names[Nb_Of_Kernels]={"K_Siglip_layernorm"};
	...
	clReleaseKernel(K_Siglip_layernorm);
