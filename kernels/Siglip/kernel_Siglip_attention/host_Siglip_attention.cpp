...
void run_custom_profiling (int Nb_Of_Kernels, int Nb_Of_Memory_Tranfers, cl_event* K_exe_event, cl_event* Mem_op_event,string* list_of_kernel_names) {
	...
	PROFILE[0].action_type="kernel";         PROFILE[0].name="K_Siglip_attention";          PROFILE[0].event = K_exe_event[0];
...
#define SIZE_SIGLIP_ENCODER_ATTENTION_EAGER_ATTN_QUERY   NUM_ATTENTION_HEADS * INPUT_DIM * HEAD_DIM
#define SIZE_SIGLIP_ENCODER_ATTENTION_EAGER_ATTN_KEY     NUM_ATTENTION_HEADS * INPUT_DIM * HEAD_DIM
#define SIZE_SIGLIP_ENCODER_ATTENTION_EAGER_ATTN_VALUE   NUM_ATTENTION_HEADS * INPUT_DIM * HEAD_DIM
#define SIZE_SIGLIP_ENCODER_ATTENTION_EAGER_ATTN_OUT     NUM_ATTENTION_HEADS * INPUT_DIM * HEAD_DIM
...
int main(int argc, char* argv[])
{
	...
	// -------------------------------------------------------------
	cl_kernel K_Siglip_attention;

	#ifdef ALL_MESSAGES
	cout << "HOST-Info: Creating a Kernel: K_Siglip_attention ..." << endl;
	#endif
	K_Siglip_attention = clCreateKernel(Program, "Siglip_attention", &errCode);
	if (errCode != CL_SUCCESS) {
		cout << endl << "HOST-Error: Failed to create K_Siglip_attention" << endl << endl;
		return EXIT_FAILURE;
	}
	...
	// ------------------------------------------------------------------
	fixed16_10 *in_q, *in_k, *in_v, *out;
	void *ptr=nullptr;

	cout << "HOST-Info: Generating data for query ... ";
	if (posix_memalign(&ptr,4096,SIZE_SIGLIP_ENCODER_ATTENTION_EAGER_ATTN_QUERY*sizeof(fixed16_10))) {
		cout << endl << "HOST-Error: Out of Memory during memory allocation for query array" << endl << endl;
		return EXIT_FAILURE;
	}
	in_q = reinterpret_cast<fixed16_10*>(ptr);
	const char* filename = "/user/undergraduate/zylin24/Test_data/in/Siglip_encoder_layer0_self_attn_q_proj_output.bin";
	load_weight(filename, in_q, SIZE_SIGLIP_ENCODER_ATTENTION_EAGER_ATTN_QUERY);
	cout << "Generated " << SIZE_SIGLIP_ENCODER_ATTENTION_EAGER_ATTN_QUERY << " values" << endl;

	cout << "HOST-Info: Generating data for key ... ";
	if (posix_memalign(&ptr,4096,SIZE_SIGLIP_ENCODER_ATTENTION_EAGER_ATTN_KEY*sizeof(fixed16_10))) {
		cout << endl << "HOST-Error: Out of Memory during memory allocation for key array" << endl << endl;
		return EXIT_FAILURE;
	}
	in_k = reinterpret_cast<fixed16_10*>(ptr);
	filename = "/user/undergraduate/zylin24/Test_data/in/Siglip_encoder_layer0_self_attn_k_proj_output.bin";
	load_weight(filename, in_k, SIZE_SIGLIP_ENCODER_ATTENTION_EAGER_ATTN_KEY);
	cout << "Generated " << SIZE_SIGLIP_ENCODER_ATTENTION_EAGER_ATTN_KEY << " values" << endl;

	cout << "HOST-Info: Generating data for value ... ";
	if (posix_memalign(&ptr,4096,SIZE_SIGLIP_ENCODER_ATTENTION_EAGER_ATTN_VALUE*sizeof(fixed16_10))) {
		cout << endl << "HOST-Error: Out of Memory during memory allocation for value array" << endl << endl;
		return EXIT_FAILURE;
	}
	in_v = reinterpret_cast<fixed16_10*>(ptr);
	filename = "/user/undergraduate/zylin24/Test_data/in/Siglip_encoder_layer0_self_attn_v_proj_output.bin";
	load_weight(filename, in_v, SIZE_SIGLIP_ENCODER_ATTENTION_EAGER_ATTN_VALUE);
	cout << "Generated " << SIZE_SIGLIP_ENCODER_ATTENTION_EAGER_ATTN_VALUE << " values" << endl;

	cout << "HOST-Info: Allocating memory for RES    ... ";
	if (posix_memalign(&ptr,4096,SIZE_SIGLIP_ENCODER_ATTENTION_EAGER_ATTN_OUT*sizeof(fixed16_10))) {
	...
	// .......................................................
	GlobMem_BUF_in_q = clCreateBuffer(Context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, SIZE_SIGLIP_ENCODER_ATTENTION_EAGER_ATTN_QUERY * sizeof(fixed16_10), in_q, &errCode);
	if (errCode != CL_SUCCESS) {
		cout << endl << "Host-Error: Failed to allocate Global Memory for GlobMem_BUF_DataIn" << endl << endl;
		return EXIT_FAILURE;
	}
	GlobMem_BUF_in_k = clCreateBuffer(Context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, SIZE_SIGLIP_ENCODER_ATTENTION_EAGER_ATTN_KEY * sizeof(fixed16_10), in_k, &errCode);
	if (errCode != CL_SUCCESS) {
		cout << endl << "Host-Error: Failed to allocate Global Memory for GlobMem_BUF_DataIn" << endl << endl;
		return EXIT_FAILURE;
	}
	GlobMem_BUF_in_v = clCreateBuffer(Context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, SIZE_SIGLIP_ENCODER_ATTENTION_EAGER_ATTN_VALUE * sizeof(fixed16_10), in_v, &errCode);
	if (errCode != CL_SUCCESS) {
		cout << endl << "Host-Error: Failed to allocate Global Memory for GlobMem_BUF_DataIn" << endl << endl;
		return EXIT_FAILURE;
	}
	GlobMem_BUF_out = clCreateBuffer(Context, CL_MEM_WRITE_ONLY | CL_MEM_USE_HOST_PTR, SIZE_SIGLIP_ENCODER_ATTENTION_EAGER_ATTN_OUT * sizeof(fixed16_10), out, &errCode);
	if (errCode != CL_SUCCESS) {
	...
	errCode |= clSetKernelArg(K_Siglip_attention,         0, sizeof(cl_mem),  &GlobMem_BUF_out);
	errCode |= clSetKernelArg(K_Siglip_attention,         1, sizeof(cl_mem),  &GlobMem_BUF_in_q);
	errCode |= clSetKernelArg(K_Siglip_attention,         2, sizeof(cl_mem),  &GlobMem_BUF_in_k);
	errCode |= clSetKernelArg(K_Siglip_attention,         3, sizeof(cl_mem),  &GlobMem_BUF_in_v);
	...
	#endif
	errCode = clEnqueueMigrateMemObjects(Command_Queue, 1, &GlobMem_BUF_in_q, 0, 0, NULL, &Mem_op_event[0]);
	if (errCode != CL_SUCCESS) {
		cout << endl << "Host-Error: Failed to write in to GlobMem_BUF_in_q" << endl << endl;
		return EXIT_FAILURE;
	}
	errCode = clEnqueueMigrateMemObjects(Command_Queue, 1, &GlobMem_BUF_in_k, 0, 0, NULL, &Mem_op_event[1]);
	if (errCode != CL_SUCCESS) {
		cout << endl << "Host-Error: Failed to write weight to GlobMem_BUF_in_k" << endl << endl;
		return EXIT_FAILURE;
	}
	errCode = clEnqueueMigrateMemObjects(Command_Queue, 1, &GlobMem_BUF_in_v, 0, 0, NULL, &Mem_op_event[2]);
	if (errCode != CL_SUCCESS) {
		cout << endl << "Host-Error: Failed to write bias to GlobMem_BUF_in_v" << endl << endl;
		return EXIT_FAILURE;
	}
	...
	cout << "HOST-Info: Submitting Kernel K_Siglip_attention ..." << endl;
	errCode = clEnqueueTask(Command_Queue, K_Siglip_attention, 0, NULL, &K_exe_event[0]);
	if (errCode != CL_SUCCESS) {
		cout << endl << "HOST-Error: Failed to submit K_Siglip_attention" << endl << endl;
		return EXIT_FAILURE;
	}
	...
	// ------------------------------------------------------
	char Output_File_Name[] = "/user/undergraduate/zylin24/Test_data/out/Siglip_encoder_layer0_eager_attention_output.txt";
	...
	for (int i=0; i < SIZE_SIGLIP_ENCODER_ATTENTION_EAGER_ATTN_OUT; i++) {
	...
	string list_of_kernel_names[Nb_Of_Kernels]={"K_Siglip_attention"};
	...
	clReleaseMemObject(GlobMem_BUF_in_q);
	clReleaseMemObject(GlobMem_BUF_in_k);
	clReleaseMemObject(GlobMem_BUF_in_v);
	clReleaseMemObject(GlobMem_BUF_out);

	clReleaseKernel(K_Siglip_attention);
