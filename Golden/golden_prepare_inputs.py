...
image = "test.jpg"
f = open("golden_prepare_inputs_debug.txt", "w")
...
## Step1: 以image_token分割input_ids，將純文字片段合併做text embedding
...
## Step2: 合併image embedding與兩段text embedding
...
# !!!Input data for phi-model!!!
prepare_inputs_labels_for_multimodal_output = new_input_embeds[0].detach().cpu().numpy().reshape(-1)
np.savetxt(f"output/prepare_inputs_labels_for_multimodal_output.txt", prepare_inputs_labels_for_multimodal_output)

# 比較拆分後和拆分前
with open("./output/golden3_phi_input.txt") as f1, open("./output/prepare_inputs_labels_for_multimodal_output.txt") as f2:
    lines1 = f1.readlines()
    lines2 = f2.readlines()
