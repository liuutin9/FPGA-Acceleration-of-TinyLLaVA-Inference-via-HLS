...
inline void load_fc1_bias(fixed32_14* out, half* bias) {
...
inline void init_fc1_out(
    ...
    half bias_0[INTERMEDIATE_SIZE / 8],
    half bias_1[INTERMEDIATE_SIZE / 8],
    half bias_2[INTERMEDIATE_SIZE / 8],
    half bias_3[INTERMEDIATE_SIZE / 8],
    half bias_4[INTERMEDIATE_SIZE / 8],
    half bias_5[INTERMEDIATE_SIZE / 8],
    half bias_6[INTERMEDIATE_SIZE / 8],
    half bias_7[INTERMEDIATE_SIZE / 8]
) {
...
void load_fc2_bias(fixed32_14* out, half* bias) {
...
inline void init_fc2_out(
    ...
    half bias_0[HIDDEN_SIZE / 8],
    half bias_1[HIDDEN_SIZE / 8],
    half bias_2[HIDDEN_SIZE / 8],
    half bias_3[HIDDEN_SIZE / 8],
    half bias_4[HIDDEN_SIZE / 8],
    half bias_5[HIDDEN_SIZE / 8],
    half bias_6[HIDDEN_SIZE / 8],
    half bias_7[HIDDEN_SIZE / 8]
) {
...
inline void load_weight(fixed32_14* out, half* weight, int len) {
	for (int i = 0; i < len; i++) {
		// #pragma HLS PIPELINE II=1
		out[i] = fixed32_14(weight[i]);
	}
}

//inline void load_fc1_weight(fixed32_14* out, half* weight) {
...
//}
//
//inline void load_fc2_weight(fixed32_14* out, half* weight) {
...
inline void compute(fixed32_14* out, fixed32_14* in, fixed32_14* weight, int len) {
    for (int i = 0; i < len; i++) {
        // #pragma HLS PIPELINE II=1
        *out += in[i] * weight[i];
    }
...
extern "C" {
    void kernel_phi_mlp(
        ...
        #pragma HLS INTERFACE m_axi port=out offset=slave bundle=gmem8 depth=2560 max_read_burst_length=256
        #pragma HLS INTERFACE m_axi port=in offset=slave bundle=gmem8 depth=2560 max_read_burst_length=256
        ...
        #pragma HLS INTERFACE m_axi port=weight_fc1_4 offset=slave bundle=gmem4 depth=3276800 max_read_burst_length=256
        #pragma HLS INTERFACE m_axi port=bias_fc1_4 offset=slave bundle=gmem4 depth=1280 max_read_burst_length=256
        #pragma HLS INTERFACE m_axi port=weight_fc1_5 offset=slave bundle=gmem5 depth=3276800 max_read_burst_length=256
        #pragma HLS INTERFACE m_axi port=bias_fc1_5 offset=slave bundle=gmem5 depth=1280 max_read_burst_length=256
        #pragma HLS INTERFACE m_axi port=weight_fc1_6 offset=slave bundle=gmem6 depth=3276800 max_read_burst_length=256
        #pragma HLS INTERFACE m_axi port=bias_fc1_6 offset=slave bundle=gmem6 depth=1280 max_read_burst_length=256
        #pragma HLS INTERFACE m_axi port=weight_fc1_7 offset=slave bundle=gmem7 depth=3276800 max_read_burst_length=256
        #pragma HLS INTERFACE m_axi port=bias_fc1_7 offset=slave bundle=gmem7 depth=1280 max_read_burst_length=256
        ...
        #pragma HLS INTERFACE m_axi port=weight_fc2_4 offset=slave bundle=gmem4 depth=3276800 max_read_burst_length=256
        #pragma HLS INTERFACE m_axi port=bias_fc2_4 offset=slave bundle=gmem4 depth=320 max_read_burst_length=256
        #pragma HLS INTERFACE m_axi port=weight_fc2_5 offset=slave bundle=gmem5 depth=3276800 max_read_burst_length=256
        #pragma HLS INTERFACE m_axi port=bias_fc2_5 offset=slave bundle=gmem5 depth=320 max_read_burst_length=256
        #pragma HLS INTERFACE m_axi port=weight_fc2_6 offset=slave bundle=gmem6 depth=3276800 max_read_burst_length=256
        #pragma HLS INTERFACE m_axi port=bias_fc2_6 offset=slave bundle=gmem6 depth=320 max_read_burst_length=256
        #pragma HLS INTERFACE m_axi port=weight_fc2_7 offset=slave bundle=gmem7 depth=3276800 max_read_burst_length=256
        #pragma HLS INTERFACE m_axi port=bias_fc2_7 offset=slave bundle=gmem7 depth=320 max_read_burst_length=256
        ...
        for (int i = 0; i < INTERMEDIATE_SIZE / 8; i++) {
            load_weight(local_weight_1, &weight_fc1_0[i * HIDDEN_SIZE], HIDDEN_SIZE);
            compute(&local_linear_1_out[i], local_linear_1_in, local_weight_1, HIDDEN_SIZE);
        }
        for (int i = 0; i < INTERMEDIATE_SIZE / 8; i++) {
        	load_weight(local_weight_1, &weight_fc1_1[i * HIDDEN_SIZE], HIDDEN_SIZE);
            compute(&local_linear_1_out[i + INTERMEDIATE_SIZE / 8], local_linear_1_in, local_weight_1, HIDDEN_SIZE);
        }
        for (int i = 0; i < INTERMEDIATE_SIZE / 8; i++) {
        	load_weight(local_weight_1, &weight_fc1_2[i * HIDDEN_SIZE], HIDDEN_SIZE);
            compute(&local_linear_1_out[i + 2 * INTERMEDIATE_SIZE / 8], local_linear_1_in, local_weight_1, HIDDEN_SIZE);
        }
        for (int i = 0; i < INTERMEDIATE_SIZE / 8; i++) {
        	load_weight(local_weight_1, &weight_fc1_3[i * HIDDEN_SIZE], HIDDEN_SIZE);
            compute(&local_linear_1_out[i + 3 * INTERMEDIATE_SIZE / 8], local_linear_1_in, local_weight_1, HIDDEN_SIZE);
        }
        for (int i = 0; i < INTERMEDIATE_SIZE / 8; i++) {
        	load_weight(local_weight_1, &weight_fc1_4[i * HIDDEN_SIZE], HIDDEN_SIZE);
            compute(&local_linear_1_out[i + 4 * INTERMEDIATE_SIZE / 8], local_linear_1_in, local_weight_1, HIDDEN_SIZE);
        }
        for (int i = 0; i < INTERMEDIATE_SIZE / 8; i++) {
        	load_weight(local_weight_1, &weight_fc1_5[i * HIDDEN_SIZE], HIDDEN_SIZE);
            compute(&local_linear_1_out[i + 5 * INTERMEDIATE_SIZE / 8], local_linear_1_in, local_weight_1, HIDDEN_SIZE);
        }
        for (int i = 0; i < INTERMEDIATE_SIZE / 8; i++) {
        	load_weight(local_weight_1, &weight_fc1_6[i * HIDDEN_SIZE], HIDDEN_SIZE);
            compute(&local_linear_1_out[i + 6 * INTERMEDIATE_SIZE / 8], local_linear_1_in, local_weight_1, HIDDEN_SIZE);
        }
        for (int i = 0; i < INTERMEDIATE_SIZE / 8; i++) {
        	load_weight(local_weight_1, &weight_fc1_7[i * HIDDEN_SIZE], HIDDEN_SIZE);
            compute(&local_linear_1_out[i + 7 * INTERMEDIATE_SIZE / 8], local_linear_1_in, local_weight_1, HIDDEN_SIZE);
        }
        ...
        for (int i = 0; i < HIDDEN_SIZE / 8; i++) {
        	load_weight(local_weight_2, &weight_fc2_0[i * INTERMEDIATE_SIZE], INTERMEDIATE_SIZE);
            compute(&local_linear_2_out[i], local_linear_2_in, local_weight_2, INTERMEDIATE_SIZE);
        }
        for (int i = 0; i < HIDDEN_SIZE / 8; i++) {
        	load_weight(local_weight_2, &weight_fc2_1[i * INTERMEDIATE_SIZE], INTERMEDIATE_SIZE);
            compute(&local_linear_2_out[i + HIDDEN_SIZE / 8], local_linear_2_in, local_weight_2, INTERMEDIATE_SIZE);
        }
        for (int i = 0; i < HIDDEN_SIZE / 8; i++) {
        	load_weight(local_weight_2, &weight_fc2_2[i * INTERMEDIATE_SIZE], INTERMEDIATE_SIZE);
            compute(&local_linear_2_out[i + 2 * HIDDEN_SIZE / 8], local_linear_2_in, local_weight_2, INTERMEDIATE_SIZE);
        }
        for (int i = 0; i < HIDDEN_SIZE / 8; i++) {
        	load_weight(local_weight_2, &weight_fc2_3[i * INTERMEDIATE_SIZE], INTERMEDIATE_SIZE);
            compute(&local_linear_2_out[i + 3 * HIDDEN_SIZE / 8], local_linear_2_in, local_weight_2, INTERMEDIATE_SIZE);
        }
        for (int i = 0; i < HIDDEN_SIZE / 8; i++) {
        	load_weight(local_weight_2, &weight_fc2_4[i * INTERMEDIATE_SIZE], INTERMEDIATE_SIZE);
            compute(&local_linear_2_out[i + 4 * HIDDEN_SIZE / 8], local_linear_2_in, local_weight_2, INTERMEDIATE_SIZE);
        }
        for (int i = 0; i < HIDDEN_SIZE / 8; i++) {
        	load_weight(local_weight_2, &weight_fc2_5[i * INTERMEDIATE_SIZE], INTERMEDIATE_SIZE);
            compute(&local_linear_2_out[i + 5 * HIDDEN_SIZE / 8], local_linear_2_in, local_weight_2, INTERMEDIATE_SIZE);
        }
        for (int i = 0; i < HIDDEN_SIZE / 8; i++) {
        	load_weight(local_weight_2, &weight_fc2_6[i * INTERMEDIATE_SIZE], INTERMEDIATE_SIZE);
            compute(&local_linear_2_out[i + 6 * HIDDEN_SIZE / 8], local_linear_2_in, local_weight_2, INTERMEDIATE_SIZE);
        }
        for (int i = 0; i < HIDDEN_SIZE / 8; i++) {
        	load_weight(local_weight_2, &weight_fc2_7[i * INTERMEDIATE_SIZE], INTERMEDIATE_SIZE);
            compute(&local_linear_2_out[i + 7 * HIDDEN_SIZE / 8], local_linear_2_in, local_weight_2, INTERMEDIATE_SIZE);
        }
