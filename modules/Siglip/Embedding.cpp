...
// output: embedded_vector[INPUT_DIM][OUTPUT_DIM] ← 如果 INPUT_DIM = n_patches，output_dim = embed_dim
void Pos_Embedding(
    float embedded_vector[INPUT_DIM][OUTPUT_DIM],
    float flatten_patched_pixel_values[INPUT_DIM][HIDDEN_SIZE]
) {
    float vision_tower_vision_tower_vision_model_embeddings_position_embedding_weight[INPUT_DIM][OUTPUT_DIM];
...
void Conv2D(
    ...
    for(int k = 0; k < HIDDEN_SIZE; k++){
        for(int i = 0; i < NUM_PATCH; i++){
            for(int j = 0; j < NUM_PATCH; j++){
                output[k][i][j] = mult_with_kernel(pixel_values, kernels[k], i * STRIDE, j * STRIDE) + bias[k];
            }
        }
    }
