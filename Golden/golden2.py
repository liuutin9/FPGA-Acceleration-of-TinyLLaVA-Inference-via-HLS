...
output_ids = model.generate(input_ids, images=image_tensor, do_sample=False, temperature=0, top_p=None, num_beams=1, pad_token_id=tokenizer.pad_token_id, max_new_tokens=512, use_cache=True)
np.savetxt(f"./output/golden2_output_ids.txt", output_ids.detach().cpu().numpy().reshape(-1))
outputs = tokenizer.batch_decode(output_ids, skip_special_tokens=True)[0]
outputs = outputs.strip()
np.savetxt("./output/golden2.txt", np.array([outputs]), fmt="%s")

# 比較拆分後和拆分前
with open("./output/golden1.txt") as f1, open("./output/golden2.txt") as f2:
    lines1 = f1.readlines()
    lines2 = f2.readlines()
